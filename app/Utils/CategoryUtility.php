...
class CategoryUtility
{
    ...
    public static function get_immediate_children($id, $with_trashed = false, $as_array = false)
    {
        ...
        $children = Cache::remember('category_of_parent_' . $id, 86400, function () use ($id) {
    ...
    public static function delete_category($id)
    {
        ...
        if (!is_null($category)) {
            ...
            $category->delete();
            self::flush_cache($category);
        }
    }
    public static function flush_cache($category)
    {
        try {
            if ($category->parent_id == 0) {
                Cache::forget('category_' . $category->id);
            } else {
                Cache::forget('category_of_parent_' . $category->id);
            }
        } catch (\Throwable $th) {
            //throw $th;
        }
