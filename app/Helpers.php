...
if (!function_exists('filter_products')) {
    function filter_products($products, $withZeroStocks = false)
    {
        $verified_sellers = verified_sellers_id();
        if ($withZeroStocks) {
            $query = $products->where('published', '1');
        } else {
            $query = $products->where('published', '1')->whereHas('stocks', function ($query) {
                $query->where('qty', '>', 0);
            });
        }
        if (get_setting('vendor_system_activation') == 1) {
            return $query->where('approved', '1')->where('auction_product', 0)->orderBy('created_at', 'desc')->where(function ($p) use ($verified_sellers) {
                $p->whereNotNull('added_by')->orWhere(function ($q) use ($verified_sellers) {
...
//filter products based on vendor activation system
if (!function_exists('cache_filter_products')) {
    function cache_filter_products($products, $cache_index, $withZeroStocks = false)
    {
        $verified_sellers = verified_sellers_id();
        if ($withZeroStocks) {
            $query = $products->where('published', '1');
        ...
        if (get_setting('vendor_system_activation') == 1) {
            return Cache::remember($cache_index, 86400, function () use ($query, $verified_sellers) {
                return $query->where('approved', '1')->where('auction_product', 0)->orderBy('created_at', 'desc')->where(function ($p) use ($verified_sellers) {
                    $p->whereNotNull('added_by')->orWhere(function ($q) use ($verified_sellers) {
                        $q->whereIn('user_id', $verified_sellers);
                    });
            ...
            return Cache::remember($cache_index, 86400, function () use ($query) {
                return $query->where('auction_product', 0)->where('added_by', 'admin')->limit(12)->get();
            });
        }
    }
}
if (!function_exists('get_parent_category_name')) {
    function get_parent_category_name($parent_id)
    {
        $all_categories = Cache::remember('all_categories_list', 86400, function () {
            return Category::all();
        });
        if ($parent_id == 0 || $parent_id == null) {
            return null;
        }
        return $all_categories->where('id', $parent_id)->first()->name;
    }
}
...
//Shows Unformated Price on page based on low to high
...
if (!function_exists('home_price_raw')) {
    function home_price_raw($product, $formatted = false)
    {
        $lowest_price = $product->unit_price;
        $highest_price = $product->unit_price;
...
//Shows Discount end time
if (!function_exists('home_discounted_end_time')) {
    function home_discounted_end_time($product)
    {
        ...
        if ($discount_applicable) {
            return date('d-m-Y H:i:s', $product->discount_end_date);
        } else {
            return '-';
        }
    }
...
function getShippingCost($carts, $index)
{
    ...
    if (get_setting('shipping_type') == 'flat_rate') {
        ...
        return Product::find($cartItem['product_id'])->shipping_cost;
    }
}

function timezones()
{
    // Return a list of timezones using PHP's built-in DateTimeZone class
...
if (!function_exists('validate_email')) {
    function validate_email($email = null)
    {
        ...
        if (preg_match($exp, $email)) {
            $mail_host = explode("@", $email);
            if (checkdnsrr(array_pop($mail_host), "MX")) {
                return true;
            } else {
                return false;
            }
